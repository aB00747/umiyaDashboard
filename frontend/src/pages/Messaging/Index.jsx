...
export default function Messaging() {
  ...
  async function openCompose() {
    try {
      ...
      // For a simple setup, just show user IDs - in production, you'd fetch all users
      setUsers([]);
    } catch { /* ignore */ }
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        ...
        <div className="lg:col-span-1 bg-white rounded-lg border border-gray-200 overflow-hidden">
          ...
          <div className="divide-y divide-gray-100 max-h-[600px] overflow-y-auto">
            {messages.length === 0 ? (
              <p className="p-6 text-sm text-gray-500 text-center">No messages</p>
            ) : (
              messages.map((msg) => (
                <button
                  key={msg.id}
                ...
                >
                  <div className="flex items-center gap-2">
                    {msg.is_read ? <MailOpen className="h-4 w-4 text-gray-400" /> : <Mail className="h-4 w-4 text-indigo-500" />}
                    <span className={classNames('text-sm', !msg.is_read ? 'font-semibold text-gray-900' : 'text-gray-700')}>
                      {msg.sender_name || `User #${msg.sender}`}
                    </span>
                  </div>
                  <p className="text-sm font-medium text-gray-900 mt-1 truncate">{msg.subject}</p>
                  <p className="text-xs text-gray-500 mt-0.5">{formatDateTime(msg.created_at)}</p>
                </button>
              ))
            )}
          </div>
            ...
            <div>
              <div className="p-5 border-b border-gray-200">
                ...
                <div className="flex items-center gap-4 mt-2 text-sm text-gray-500">
                  <span>From: <span className="font-medium text-gray-700">{selected.sender_name || `User #${selected.sender}`}</span></span>
                  <span>To: <span className="font-medium text-gray-700">{selected.recipient_name || `User #${selected.recipient}`}</span></span>
              ...
              {selected.replies && selected.replies.length > 0 && (
                <div className="px-5 pb-5 space-y-3">
                  <h4 className="text-sm font-semibold text-gray-900">Replies</h4>
                  {selected.replies.map((r) => (
                    <div key={r.id} className="p-3 bg-gray-50 rounded-lg text-sm">
                      <div className="flex items-center justify-between mb-1">
                        <span className="font-medium text-gray-900">{r.sender_name}</span>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
          <div className="bg-white rounded-xl shadow-xl w-full max-w-lg m-4">
            <div className="flex items-center justify-between p-5 border-b border-gray-200">
              ...
              <button onClick={() => setComposeOpen(false)} className="p-1 hover:bg-gray-100 rounded"><X className="h-5 w-5" /></button>
            </div>
            <form onSubmit={handleSend} className="p-5 space-y-4">
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">Recipient User ID *</label>
                <input type="number" required className="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm" value={form.recipient} onChange={(e) => setForm({ ...form, recipient: e.target.value })} placeholder="Enter user ID" />
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">Subject *</label>
                <input type="text" required className="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm" value={form.subject} onChange={(e) => setForm({ ...form, subject: e.target.value })} />
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">Message *</label>
                <textarea required rows={5} className="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm" value={form.body} onChange={(e) => setForm({ ...form, body: e.target.value })} />
              </div>
              <div className="flex justify-end gap-3">
                <button type="button" onClick={() => setComposeOpen(false)} className="px-4 py-2 border border-gray-300 rounded-lg text-sm hover:bg-gray-50">Cancel</button>
