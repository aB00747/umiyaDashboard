  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-gray-500 mt-1">Welcome to your operational overview</p>
      </div>
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-gray-200">
                  <th className="text-left py-2 px-2 font-medium text-gray-500">Order</th>
              ...
              <tbody>
                {recent_orders.map((o) => (
                  <tr key={o.id} className="border-b border-gray-100">
                    <td className="py-2.5 px-2 font-medium text-gray-900">{o.order_number}</td>
                    <td className="py-2.5 px-2 text-gray-600">{o.customer_name}</td>
                    <td className="py-2.5 px-2 text-gray-900">{formatCurrency(o.total_amount)}</td>
                    <td className="py-2.5 px-2">
                      <span className={`inline-flex px-2 py-0.5 rounded-full text-xs font-medium ${statusColors[o.status] || 'bg-gray-100 text-gray-800'}`}>
                        {o.status}
                      </span>
                    </td>
                  </tr>
            ...
            {low_stock_items.map((item, i) => (
              <div key={i} className="flex items-center justify-between p-3 bg-red-50 rounded-lg border border-red-100">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <p className="text-sm font-medium text-gray-900">{item.chemical_name}</p>
