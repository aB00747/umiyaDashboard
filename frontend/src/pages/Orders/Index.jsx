  ...
  return (
    <div className="space-y-4">
      ...
      <div className="bg-white rounded-lg border border-gray-200 p-4 flex flex-wrap gap-3">
        <select className="px-3 py-2 border border-gray-300 rounded-lg text-sm" value={filters.status} onChange={(e) => setFilters({ ...filters, status: e.target.value, page: 1 })}>
          <option value="">All Status</option>
          {['pending', 'confirmed', 'processing', 'shipped', 'delivered', 'cancelled'].map((s) => <option key={s} value={s}>{s}</option>)}
        </select>
        <select className="px-3 py-2 border border-gray-300 rounded-lg text-sm" value={filters.payment_status} onChange={(e) => setFilters({ ...filters, payment_status: e.target.value, page: 1 })}>
      ...
      <div className="bg-white rounded-lg border border-gray-200 overflow-hidden">
          ...
          <tbody>
            {loading ? (
              ...
              <tr key={o.id} className="border-b border-gray-100 hover:bg-gray-50">
                <td className="py-3 px-4 font-medium text-gray-900">{o.order_number}</td>
                <td className="py-3 px-4 text-gray-600">{o.customer_name}</td>
                <td className="py-3 px-4 text-gray-600">{formatDate(o.order_date)}</td>
                <td className="py-3 px-4 text-right font-medium text-gray-900">{formatCurrency(o.total_amount)}</td>
                ...
                <td className="py-3 px-4 text-right">
                  <button onClick={() => viewDetails(o.id)} className="p-1.5 text-gray-400 hover:text-indigo-600 rounded"><Eye className="h-4 w-4" /></button>
                </td>
              </tr>
            ))}
          </tbody>
        ...
        <div className="flex items-center justify-between px-4 py-3 border-t border-gray-200">
          ...
          <div className="flex gap-2">
            <button disabled={filters.page <= 1} onClick={() => setFilters({ ...filters, page: filters.page - 1 })} className="p-2 border border-gray-300 rounded-lg text-sm disabled:opacity-50 hover:bg-gray-50"><ChevronLeft className="h-4 w-4" /></button>
            <button disabled={filters.page >= totalPages} onClick={() => setFilters({ ...filters, page: filters.page + 1 })} className="p-2 border border-gray-300 rounded-lg text-sm disabled:opacity-50 hover:bg-gray-50"><ChevronRight className="h-4 w-4" /></button>
          </div>
        </div>
      </div>
      ...
      {/* View Detail Panel */}
      {viewOrder && (
        <div className="fixed inset-y-0 right-0 w-[480px] bg-white shadow-xl border-l border-gray-200 z-40 overflow-y-auto">
          <div className="flex items-center justify-between p-4 border-b border-gray-200">
            <h3 className="font-semibold text-gray-900">{viewOrder.order_number}</h3>
          ...
          <div className="p-4 space-y-4">
            <div className="grid grid-cols-2 gap-3 text-sm">
              ...
              <div><span className="text-gray-500">Date:</span> <span className="font-medium">{formatDate(viewOrder.order_date)}</span></div>
              <div><span className="text-gray-500">Total:</span> <span className="font-medium">{formatCurrency(viewOrder.total_amount)}</span></div>
              <div><span className="text-gray-500">Tax:</span> <span className="font-medium">{formatCurrency(viewOrder.tax_amount)}</span></div>
            </div>
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">Update Status</label>
              <select className="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm" value={viewOrder.status} onChange={(e) => updateStatus(viewOrder.id, e.target.value)}>
                {['pending', 'confirmed', 'processing', 'shipped', 'delivered', 'cancelled'].map((s) => <option key={s} value={s}>{s}</option>)}
              </select>
            </div>
            <div>
          ...
          <div className="bg-white rounded-xl shadow-xl w-full max-w-3xl max-h-[90vh] overflow-y-auto m-4">
            ...
            <form onSubmit={handleSave} className="p-5 space-y-4">
                ...
                <div className="space-y-2">
                  {form.items.map((item, i) => (
                    <div key={i} className="flex items-center gap-3 p-3 bg-gray-50 rounded-lg">
                      <select className="flex-1 px-2 py-1.5 border border-gray-300 rounded text-sm" value={item.chemical} onChange={(e) => updateItem(i, 'chemical', e.target.value)}>
                        <option value="">Select Chemical</option>
                        {chemicals.map((c) => <option key={c.id} value={c.id}>{c.chemical_name} ({formatCurrency(c.selling_price)})</option>)}
                      </select>
                      <input type="number" min="0.01" step="any" placeholder="Qty" className="w-20 px-2 py-1.5 border border-gray-300 rounded text-sm" value={item.quantity} onChange={(e) => updateItem(i, 'quantity', e.target.value)} />
                      <input type="number" min="0" step="any" placeholder="Price" className="w-28 px-2 py-1.5 border border-gray-300 rounded text-sm" value={item.unit_price} onChange={(e) => updateItem(i, 'unit_price', e.target.value)} />
                      <span className="w-28 text-sm font-medium text-right">{formatCurrency((parseFloat(item.quantity) || 0) * (parseFloat(item.unit_price) || 0))}</span>
                      {form.items.length > 1 && (
              ...
              <div>
                ...
                <textarea className="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm" rows={2} value={form.notes} onChange={(e) => setForm({ ...form, notes: e.target.value })} />
              </div>
